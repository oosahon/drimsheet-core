...
import makeGiftsAccountService from '../gifts.service';
...
describe('giftsAccountService', () => {
  ...
  it('creates a sub-account under a gifts control account', async () => {
    ...
    mockLedgerAccountRepo.findLatestBySubType.mockResolvedValueOnce({
      id: generateUUID(),
      code: '408099',
      materializedPath: `${controlAccount.materializedPath}.408099`,
    });
    ...
    expect(account).toMatchObject({
      ...
      code: '408100',
      materializedPath: `${controlAccount.materializedPath}.408100`,
      accountingEntityId: accountingEntity.id,
      normalBalance: ENormalBalance.Credit,
      type: ELedgerType.Revenue,
      subType: ERevenueSubType.Gifts,
  ...
  it('allocates the first sub-account code when no later account exists', async () => {
    ...
    expect(account.materializedPath).toBe(
      `${controlAccount.materializedPath}.408001`
    );
  });
  ...
  it.each([
    ...
    async ({ overrides }) => {
      mockLedgerAccountRepo.findByCode.mockResolvedValueOnce(
        makeControlAccount(ERevenueAccountBehavior.Gifts, overrides)
      );
