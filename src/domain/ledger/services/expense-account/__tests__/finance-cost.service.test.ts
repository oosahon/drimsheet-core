...
import makeFinanceCostAccountService from '../finance-cost.service';
...
describe('financeCostAccountService', () => {
  ...
  it.each([
    ...
    async (controlAccountBehavior) => {
      ...
      mockLedgerAccountRepo.findLatestBySubType.mockResolvedValueOnce({
        id: generateUUID(),
        code: `${EXPENSE_LEDGER_CODES.FINANCE_COST.PREFIX}099`,
        materializedPath: `${controlAccount.materializedPath}.${EXPENSE_LEDGER_CODES.FINANCE_COST.PREFIX}099`,
      });
      ...
      expect(account).toMatchObject({
        ...
        code: `${EXPENSE_LEDGER_CODES.FINANCE_COST.PREFIX}100`,
        materializedPath: `${controlAccount.materializedPath}.${EXPENSE_LEDGER_CODES.FINANCE_COST.PREFIX}100`,
        accountingEntityId: accountingEntity.id,
        normalBalance: ENormalBalance.Debit,
        type: ELedgerType.Expense,
        subType: EExpenseSubType.FinanceCost,
  ...
  it('allocates the first sub-account code when no later account exists', async () => {
    ...
    expect(account.code).toBe(`${EXPENSE_LEDGER_CODES.FINANCE_COST.PREFIX}001`);
    expect(account.materializedPath).toBe(
      `${controlAccount.materializedPath}.${EXPENSE_LEDGER_CODES.FINANCE_COST.PREFIX}001`
    );
  });
  ...
  it.each([
    ...
    async ({ overrides }) => {
      mockLedgerAccountRepo.findByCode.mockResolvedValueOnce(
        makeControlAccount(EExpenseAccountBehavior.FinanceCost, overrides)
      );
