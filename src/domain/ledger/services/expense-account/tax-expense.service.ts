...
import { ITaxExpenseAccountService } from '../../types/tax-expense.service.types';
...
function makeCreateHeader(
  deps: IDependencies
): ITaxExpenseAccountService['createHeader'] {
...
function makeCreateSubAccount(
  ...
  return async (payload, repoOptions) => {
    ...
    const { controlAccount, factoryContext } =
      await controlAccountResolverHelper<TIncomeTaxLedgerCode>({
    ...
    const materializedPath =
      ledgerAccountEntity.getMaterializedPath<TIncomeTaxLedgerCode>(
