...
describe('vars config', () => {
  ...
  it('uses local-safe defaults outside the test environment', () => {
    ...
    expect(vars).toEqual({
      ...
      METRICS_EXPORT_INTERVAL_MS: undefined,
      METRICS_OTLP_HTTP_ENDPOINT: '',
      METRICS_SHUTDOWN_TIMEOUT_MS: undefined,
  ...
  it('maps explicit environment values into an immutable config', () => {
    const vars = loadVars({
      ...
      SENTRY_FLUSH_TIMEOUT_MS: '4000',
      SENTRY_TRACE_PROPAGATION_TARGETS: 'https://api.example.com',
