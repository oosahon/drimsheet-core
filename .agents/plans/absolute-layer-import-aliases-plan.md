...
| Rewrite production aliases after emit       | Build tooling; explicit plan decision and plain-Node `start` precedent                            | `package.json`, `package-lock.json`, `scripts/check-built-import-aliases.mjs`; canonical build and emitted-output scan         | Complete — canonical Yarn build, emitted-output scan, and plain-Node graph load pass                                                                                                             |
...
| Reconcile and verify                        | Repository-wide                                                                                   | Lint, test type-check, Jest, build, emitted alias scan, plain-Node graph load                                                  | Complete — lint, type-check, 2,055 tests, canonical build, emitted scan, and graph load pass                                                                                                     |

## Approved Deviation

- Added the five alias mappings to `tsoa.json` after the user approved fixing
  the reproduced canonical-build failure. TSOA does not inherit compiler
  options from `tsconfig.json`; generated artifact contents, paths, and imports
  remain outside the implementation diff.
