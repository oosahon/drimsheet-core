...
The following table describes the key actors and internal domain boundaries that interact with PurpleLedger.
...
| **Purple Ledger Core**  | System Boundary         | The central accounting system that encapsulates business logic, double-entry accounting rules, and tax computations.                                                          |
...
The following table describes the technical interfaces, protocols, and infrastructural systems that link PurpleLedger Core to its environment.
...
| **🤖 AI Agents**           | External System | MCP          | Agents utilizing the Model Context Protocol (MCP) to interact securely with the PurpleLedger exposed toolsets. |
| **Purple Ledger Core API** | Core System     | -            | The central Node.js backend application containing all business logic and immutable transaction processing.    |
